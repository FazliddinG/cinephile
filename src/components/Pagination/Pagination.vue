<template>
    <div class="pagin" :class="{ pagin_top: top }">
        <ul class="pagin__menu">
            ...
            <li class="pagin__num" v-if="startLinks" v-for="item in startLinks" :key="item" @click="changePage(item)"
                :class="{ active: item == currentPage }">
                {{ item }}
            </li>
            <li class="pagin__num" v-if="prevDots" @click="changePage(prevDots)">...</li>
            <li class="pagin__num" v-if="centerLinks" v-for="item in centerLinks" :key="item" @click="changePage(item)"
                :class="{ active: item == currentPage }">
                {{ item }}
            </li>
            <li class="pagin__num" v-if="nextDots" @click="changePage(nextDots)">...</li>
            <li class="pagin__num" v-if="endLinks" v-for="item in endLinks" :key="item" @click="changePage(item)"
                :class="{ active: item == currentPage }">
                {{ item }}
            </li>
...
function createNums(page) {
    ...
    endLinks.value = nextDots.value ? [lastPage--, lastPage].reverse() :
        page == lastPage - 3 ? [lastPage--, lastPage--, lastPage--, lastPage--, lastPage].reverse() :
